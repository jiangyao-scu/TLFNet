...
class Eval_thread():
    ...
    def run(self):
        ...
        # max_e = 0
        # s = 0
        self.LOG('{} {} S: {:.4f} max-F: {:.4f} max-E: {:.4f} M: {:.4f} number: {}\n'.format(self.method, self.dataset, s, max_f, max_e, mae, len(self.loader)))
        return '{} {} S: {:.4f} max-F: {:.4f} max-E: {:.4f} M: {:.4f} number: {}\n'.format(self.method, self.dataset, s, max_f, max_e, mae, len(self.loader))
    def Eval_mae(self):
        ...
        with torch.no_grad():
            ...
            for pred, gt in self.loader:
                if self.cuda:
                    ...
                    pred = trans(pred)
                    gt = trans(gt)
                # a = pred[0].cpu().detach().numpy().copy()
                # b = gt [0].cpu().detach().numpy().copy()
